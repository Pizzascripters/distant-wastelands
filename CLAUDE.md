...
Automated tests must be run with `./tools/test.sh` only. That script starts a private Xvfb and never uses the host display. Optional positional filters select files or cases; `./tools/test.sh` with no arguments still runs the full suite. Do not run `godot --headless` for the test runner, and do not launch Godot against `DISPLAY=:0` or the session Wayland/X11 display while testing.
